...
[u(:,ti), ut(:,ti), pz(1,ti), vz(1,ti), theta(1,ti), omega(1,ti), force(1,ti), torque(1,ti), vel(1,ti)] =  RK4(u(:,ti-1), ut(:,ti-1),pz(1,ti-1),vz(1,ti-1),theta(1,ti-1), omega(1,ti-1), xdim,  spatInc, tempInc, xghost, m_p, rho, m, Jx);
end
%% Display Functions
...
function [u_n, ut_n, pz_n, vz_n, theta_n, omega_n, force, torque, vel] = RK4(u_ti, ut_ti, pz, vz, theta, omega, xdim, spatInc, tempInc, xghost, m_p, rho, m, Jx)
...
T = (p(2) * (p(1)-3)* p(5) + p(4))*p(8); %(L*rho+m_p)*g

fx3 = p(8).*ux + p(8).*xghost.*uxx + p(4)*p(8).*uxx./p(5);     
fx3(p(1)-1) = (Fx- ux(p(1)-1)*T)/p(6); %Enforce BC
fx3(2) = p(8)*ux(2);
end
...
Kp = 1.5*0.75;  %Gain matrix of Lyapunov based feedback
Kd = 1 * 0.75;
...
% vel_ref = -Kd * ux(p(1)-1)- Kp* u(p(1)-1,1); %Turn into straight PD
if isnan(vel_ref)
    vel_ref = 0;
end
end
...
% K = [0,3,0,4.5,0,0; 0,0,0.5,0,1.5,0.25]; 
...
%% PRINTOUT
%Function to generate state plots. Not implemented into animation selection
function [] = Hi()
subplot(6,2,1)
hold on
plot(t, u(xdim-1,:))
...
hold on
plot(t, force)
title('Quadrotor Thrust Input')
xlabel('Time')
ylabel('Force')

subplot(6,2,8)
hold on
plot(t, movmean(torque,300))
title('Quadrotor Torque Input')
xlabel('Time')
ylabel('Force')
